    ...
    >
      ...
      >
        <motion.div layoutId={`card-${selected.id}`}>
          <img src={selected.featured_image.large} />
        </motion.div>
        <motion.div
          initial={{
            opacity: 0,
        ...
        >
          ...
          <p className="my-4">{parse(selected.overview)}</p>
          <button className="btn btn-primary btn-block" onClick={()=>{window.open(selected.project_link, '_blank')}}>Launch project</button>
        </motion.div>
      </div>
    </div>
  );
